...
PYLINT=pylint
RM=rm -rf
SOURCES_ANALYSIS=deployment test
SOURCES_COVERAGE=deployment,test
...
get_init_version:
	$(eval INIT_VERSION=v$(shell grep __version__ deployment/__init__.py | sed -E "s/__version__ = .([0-9.]+)./\\1/"))
...
clean:
	...
	$(RM) src/ build/ dist/ gros-deployer.egg-info/
